...
from .toolkit import FixedPathPlanner, CsvPathPlanner, get_location_distance, get_vehicle_pos
...
class CarlaWptFixedEnv(CarlaWptEnv):
    ...
    * ``max_flow_dist``: Maximum distance between two cars in the flow
    ...
    def on_reset(self) -> None:
        ...
        # New episode, new ego actor.
        # Do not reuse PolyPlannerController from the previous episode.
        self._clear_waypoint_controller_cache()

        route_csv = self._config.get("route_csv", None)

        if route_csv is not None and route_csv != "":
            self.ego_planner = CsvPathPlanner(
                vehicle=self.ego,
                csv_path=route_csv,
                max_waypoints=self._config.get("planner_max_waypoints", 60),
                reach_threshold=self._config.get("planner_reach_threshold", 0.8),
            )
        else:
