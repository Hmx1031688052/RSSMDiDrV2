...
class BirdeyeRenderer:
    ...
    def _render_global_path(self, **env_state):
        """
        Render the full ego global path loaded from CSV.
        This path is not popped by planner progress.
        """
        path = env_state.get("ego_global_path", None)
        if path is None or len(path) < 2:
            return

        color = env_state.get("global_path_color", Color.SCARLET_RED_0)

        points = np.array(
            [
                self._world_to_pixel(carla.Location(x=float(p[0]), y=float(p[1])))
                for p in path
            ],
            dtype=np.int32,
        )
        ...
        # 可选：画起点和终点，方便调试
        cv2.circle(self._surface, tuple(points[0]), 8, Color.CHAMELEON_0, -1)
        cv2.circle(self._surface, tuple(points[-1]), 8, Color.SCARLET_RED_0, -1)
    ...
    _RENDER_METHODS = {
        ...
        BirdeyeEntity.GLOBAL_PATH: _render_global_path,  # 新增
